...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen,
                  gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, aux_branch, num_classes, \
    ...
    if local_rank == 0:
        ...
        if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
            print('Save best model to m.pth')
            torch.save(model.state_dict(), os.path.join(save_dir, "m.pth"))
