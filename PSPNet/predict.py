...
if __name__ == "__main__":
    ...
    dir_save_npy_path  = 'npy/img_npy_out'
    #-------------------------------------------------------------------------#
    ...
    if mode == "predict":
        ...
        for img_name in tqdm(img_names):
            if img_name.lower().endswith(('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
                ...
                if not os.path.exists(dir_save_npy_path):
                    os.makedirs(dir_save_npy_path)

                # 将处理后的图像转换为NumPy数组
                r_image_array = np.array(r_image)

                # 保存NumPy数组为.npy格式
                np.save(os.path.join(dir_save_npy_path, f'{os.path.splitext(img_name)[0]}.npy'), r_image_array)
