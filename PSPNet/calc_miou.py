...
def compute_mIoU(gt_dir, pred_dir):
    ...
    for f in os.listdir(pred_dir):
        if f.endswith('.npy'):
            parts = f.split('_')
            # 找到最后一个下划线后面的部分，即文件编号
            file_number = parts[-1].split('.')[0]
            npy_name_list.append(file_number)
    #npy_name_list = [f.split('_')[1].split('.')[0] for f in os.listdir(pred_dir) if f.endswith('.npy')]
    #npy_name_list = [os.path.splitext(f)[0] for f in os.listdir(pred_dir) if f.endswith('.npy')]
    # 修改命名规则以匹配文件名
